...
def temp_multiplier(predicted_actions):
    mean_val = torch.mean(predicted_actions).item()  # Get the max value as a Python float
...
class SmoothedParamLoss(nn.Module):
    ...
    def forward(self, predicted_rewards, target_rewards, step_count):
        ...
        # noisy_actions = predicted_rewards + predicted_rewards * torch.randn_like(predicted_rewards) * self.noise_std
        noisy_action_pred = F.softmax(noisy_actions / (self.temperature * temp_multiplier(predicted_rewards)), dim=-1)
        ...
        entropy = -torch.sum(noisy_action_pred * torch.log(noisy_action_pred + 1e-8), dim=-1).mean()

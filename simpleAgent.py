...
class SimpleAgent(mesa.Agent):
    def __init__(self, unique_id, wealth, environment):
        ...
        # Gov variables
        self.tau_income = self.environment.tau_income if self.gov else 0
        self.tau_wealth = self.environment.tau_wealth if self.gov else 0
    ...
    def update_frozen_state(self):
        frozen_state = [self.wealth, self.alpha_demand, self.alpha_supply, self.beta_demand, self.beta_supply, self.v, self.psi]
        return frozen_state
    
    def update_from_frozen_state(self):
        self.wealth, self.alpha_demand, self.alpha_supply, self.beta_demand, self.beta_supply, self.v, self.psi = self.frozen_state
    ...
    def action_outcomes(self, action, forward=False):
        if action == 0: # subsistence
            ...
            income = self.interest_rate * self.wealth + self.v * (1-self.tau_income)
            ...
            income = self.wealth * self.interest_rate + (self.mu * self.investment_needed * (r - self.interest_rate) - self.mu * self.v) * (1-self.tau_income)
            reward = self.utility(income, 1)
        else: 
        ...
        if not forward:
            if (
                ...
                (action == 3 and self.wealth < self.investment_needed * self.mu) or
    ...
    def simulate_path(self):
        ...
        # Update states
        self.wealth = max(self.wealth + income, 0)

...
class Environment(mesa.Model):
    ...
    def __init__(self, N, think=5, aging=True, gov=False):
        super().__init__()
        ...
        self.horizon = 3 if self.gov else 5
        ...
        mean, sigma = 6, 1
        scale = np.exp(mean)
        wealth_values = lognorm.rvs(sigma, scale=scale, size=self.num_agents, random_state=4)
        wealth_values.sort()
        #wealth_values = [600] * (self.num_agents // 2) + [5] * (self.num_agents // 2)
        #wealth_values = [600] * self.num_agents
        #wealth_values = [3000] * int(1*self.num_agents/(self.mu+1)) + [1] * (self.num_agents - int(1*self.num_agents/(self.mu+1)))
    ...
    def step(self):
        ...
        if not self.simulate and self.gov:
    ...
    def run(self, steps, tau_income=None, tau_wealth=None, simulate=False):
    ...
    def plot(self):
        ...
        # ---- Middle-left: Pdf starting and final ----
        if variable_name.capitalize() in data.columns:
            ...
            axes[1, 1].plot(unique_steps, gini_over_time, color='red', linewidth=2, label="Gini Coefficient")
        ...
        for agent_id in df_pivot.columns:
            ...
            axes[2, 0].plot(df_pivot.index, df_pivot[agent_id], alpha=0.3, color='black')
        ...
        if variable_name.capitalize() in data.columns:
            ...
            for agent_id, agent_data in data[variable_name.capitalize()].groupby(level="AgentID"):
                occupation = data.loc[data.index.get_level_values('AgentID') == agent_id, 'Occupation'].iloc[0]
                axes[2, 1].plot(agent_data.index.get_level_values("Step"), agent_data.values, alpha=0.5, color='black')
            ...
            time_steps = model_data.index
            wage_v = model_data["Wage"]

            # Plot wage v on the secondary y-axis
            ax2.plot(time_steps, wage_v, color='blue', linewidth=2, label="Wage")
